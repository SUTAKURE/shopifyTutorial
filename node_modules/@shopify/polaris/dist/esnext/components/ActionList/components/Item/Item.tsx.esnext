import React__default from 'react';
import { useFeatures } from '../../../../utilities/features/hooks.tsx.esnext';
import { classNames } from '../../../../utilities/css.ts.esnext';
import { UnstyledLink as UnstyledLink$1 } from '../../../UnstyledLink/UnstyledLink.tsx.esnext';
import { Icon as Icon$1 } from '../../../Icon/Icon.tsx.esnext';
import { Scrollable as Scrollable$1 } from '../../../Scrollable/Scrollable.tsx.esnext';
import { Badge as Badge$1 } from '../../../Badge/Badge.tsx.esnext';
import { TextStyle as TextStyle$1 } from '../../../TextStyle/TextStyle.tsx.esnext';
import styles from '../../ActionList.scss.esnext';

var _ref = /*#__PURE__*/React__default.createElement(Scrollable$1.ScrollTo, null);

function Item({
  id,
  badge,
  content,
  accessibilityLabel,
  helpText,
  url,
  onAction,
  icon,
  image,
  disabled,
  external,
  destructive,
  ellipsis,
  active,
  role
}) {
  const {
    newDesignLanguage
  } = useFeatures();
  const className = classNames(styles.Item, disabled && styles.disabled, destructive && styles.destructive, active && styles.active, newDesignLanguage && styles.newDesignLanguage);
  let imageElement = null;

  if (icon) {
    imageElement = /*#__PURE__*/React__default.createElement("div", {
      className: styles.Image
    }, /*#__PURE__*/React__default.createElement(Icon$1, {
      source: icon
    }));
  } else if (image) {
    imageElement = /*#__PURE__*/React__default.createElement("div", {
      role: "presentation",
      className: styles.Image,
      style: {
        backgroundImage: `url(${image}`
      }
    });
  }

  const contentText = ellipsis && content ? `${content}…` : content;
  const contentMarkup = helpText ? /*#__PURE__*/React__default.createElement("div", null, /*#__PURE__*/React__default.createElement("div", null, contentText), /*#__PURE__*/React__default.createElement(TextStyle$1, {
    variation: "subdued"
  }, helpText)) : contentText;
  const badgeMarkup = badge && /*#__PURE__*/React__default.createElement("span", {
    className: styles.BadgeWrapper
  }, /*#__PURE__*/React__default.createElement(Badge$1, {
    status: badge.status
  }, badge.content));
  const textMarkup = imageElement ? /*#__PURE__*/React__default.createElement("div", {
    className: styles.Text
  }, contentMarkup) : contentMarkup;
  const contentElement = /*#__PURE__*/React__default.createElement("div", {
    className: styles.Content
  }, imageElement, textMarkup, badgeMarkup);
  const scrollMarkup = active ? _ref : null;
  const control = url ? /*#__PURE__*/React__default.createElement(UnstyledLink$1, {
    id: id,
    url: url,
    className: className,
    external: external,
    "aria-label": accessibilityLabel,
    onClick: onAction
  }, contentElement) : /*#__PURE__*/React__default.createElement("button", {
    id: id,
    type: "button",
    className: className,
    disabled: disabled,
    "aria-label": accessibilityLabel,
    onClick: onAction
  }, contentElement);
  return /*#__PURE__*/React__default.createElement("li", {
    role: role,
    "aria-selected": active
  }, scrollMarkup, control);
}

export { Item };
